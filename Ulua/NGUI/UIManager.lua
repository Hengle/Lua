...
function UIManager:IsAllSonView(views)
    ...
    for i, v in ipairs(views) do
        if not self:IsSonView(v) then -- 检测ui层级配置中是否是1或2
            isAllSonView = false;
            break;
        end
    end
...
function UIManager:SaveLevelBackWindowStackListData(isSaveToSource)
    if LevelManager.enterGameSceneType ~= EnterGameSceneType.Lobby then
        return;
    end
    local toViewName = "";
...
-- region 通用背板的处理
function UIManager:InitCommonBkg(pos,call)
    ...
    self.common_bkg_requestId = CreateInstanceAsync(nil,self.common_bkg_res,false,function(res,instance)
        ...
        if instance then
            ...
            self:RepositionCommonBkgCamera(pos);
            if call then
                call();
            end
        end
...
function UIManager:ShowCommonBkg(pos)
    -- 如果存在直接显示
    if self.common_bkg_instanceId == nil then
        self:InitCommonBkg(pos,function()
            self:ShowCommonBkg(pos);
        end);
    else
        local go = GetGameObjectByID(self.common_bkg_instanceId);
        if go then
...
function UIManager:RepositionCommonBkgCamera(pos)
    if not pos then
